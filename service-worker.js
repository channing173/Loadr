...
// Helper to get active cache name (fallback to v0.81, must match version.json)
function currentCacheName() {
  return self.CACHE_NAME || 'weight-converter-v0.81';
}
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    fetch('./version.json', { cache: 'no-store' })
      ...
      .catch(() => {
        // fallback to v0.81 (must match version.json)
        const CACHE_NAME = 'weight-converter-v0.81';
        self.CACHE_NAME = CACHE_NAME;
        return caches.open(CACHE_NAME).then(cache => cache.addAll(urlsToCache));
      })
  );
